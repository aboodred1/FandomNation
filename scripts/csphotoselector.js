...
var CSPhotoSelector = (function(module, $) {
	...
	// Public functions
	var init, setAlbums, addVideoAlbum, getAlbums, getAlbumById, getPhotoById, getVideoById, setPhotos, setVideos, newInstance,

	// Private variables
	settings, albums, prev, photos, videos,
	$albums, $photos, $videos, $container, $albumsContainer, $photosContainer, $videosContainer, $selectedCount, $selectedCountMax, 
	$pageNumber, $pageNumberTotal, $pagePrev, $pageNext, $buttonClose, $buttonOK, $buttonCancel,

	// Private functions
	$getAlbumById, $getPhotoById, $getVideoById, buildAlbumSelector, buildPhotoSelector, sortPhotos, log, htmlEntities;
	...
	init = function(options) {
		...
		$videosContainer	= $container.find(settings.photosContainerSelector);
	...
	/*addVideoAlbum = function(){
		albums = getAlbums();
		console.log('addVideoAlbum');
		console.log(albums);
		albums.unshift({
			"id": "videos",
			"from": {
	...
	getVideoById = function(id) {
		var i, len;
		id = id.toString();
		for (i = 0, len = videos.length; i < len; i += 1) {
			if (videos[i].id === id) {
				return videos[i];
			}
		}
	...
	newInstance = function(options) {
		// Public functions
		var showAlbumSelector, showPhotoSelector, showVideoSelector, hidePhotoSelector, hideAlbumSelector, getselectedAlbumIds, getselectedPhotoIds, setDisabledPhotoIds, reset,

		// Private variables
		instanceSettings, selectedAlbumIds = [], selectedPhotoIds = [], disabledPhotoIds = [],

		// Private functions
		bindEvents, unbindEvents, updateAlbumContainer, updatePhotosContainer, updateVideosContainer, updatePaginationButtons, selectAlbum, selectPhotos, selectVideos;
		...
		instanceSettings = {
			...
			callbackVideoSelected	: null,
			callbackPhotoUnselected	: null,
			callbackVideoUnselected	: null,
		...
		showVideoSelector = function(callback, albumId) {
			var i, len;
			log('CSPhotoSelector - show Videos');
			log('$videos: ', $videos);
			log('videos: ', videos);
			...
			if (!$videos || albumId) {
				return buildVideoSelector(function() {
					showVideoSelector(callback);
				}, albumId);
			} else {
				// Update classnames to represent the selections for this instance
				$videos.removeClass(settings.albumSelectedClass + ' ' + settings.albumDisabledClass + ' ' + settings.photoFilteredClass);
				for (i = 0, len = videos.length; i < len; i += 1) {
					if ($.inArray(videos[i].id, selectedPhotoIds) !== -1) {
						$($videos[i]).addClass(settings.albumSelectedClass);
					}
					if ($.inArray(videos[i].id, disabledPhotoIds) !== -1) {
						$($videos[i]).addClass(settings.albumDisabledClass);
					}
				}
				// Update paging
				$selectedCount.html(selectedPhotoIds.length);
				$selectedCountMax.html(instanceSettings.maxSelection);
				updateVideosContainer(1);
				// updatePaginationButtons(1);
				$container.fadeIn(100);
				if (typeof callback === 'function') {
					callback();
		...
		unbindEvents = function() {
			...
			$videosContainer.children().unbind('click');
		...
		// Set the contents of the videos container
		updateVideosContainer = function(pageNumber) {
			var firstIndex, lastIndex;
			firstIndex = (pageNumber - 1) * instanceSettings.photosPerPage;
			lastIndex = pageNumber * instanceSettings.photosPerPage;
			$videosContainer.html($videos.not("." + settings.photoFilteredClass).slice(firstIndex, lastIndex));
			$videosContainer.children().bind('click', function(e) {
				e.preventDefault();
				selectVideos($(this));
			});
		};
		...
		selectVideos = function($video) {
			var videoId, i, len, removedId;
			videoId = $video.attr('data-id');

			// If the Video is disabled, ignore this
			if ($video.hasClass(settings.albumDisabledClass)) {
				return;
			}

			if (!$video.hasClass(settings.albumSelectedClass)) {
				// If autoDeselection is enabled and they have already selected the max number of Videos, deselect the first Video
				if (instanceSettings.autoDeselection && selectedPhotoIds.length === instanceSettings.maxSelection) {
					removedId = selectedPhotoIds.splice(0, 1);
					$getVideoById(removedId).removeClass(settings.albumSelectedClass);
					$selectedCount.html(selectedPhotoIds.length);
				}
				if (selectedPhotoIds.length < instanceSettings.maxSelection) {
					// Add Video to selectedPhotoIds
					if ($.inArray(videoId, selectedPhotoIds) === -1) {
						selectedPhotoIds.push(videoId);
						$video.addClass(settings.albumSelectedClass);
						$selectedCount.html(selectedPhotoIds.length);
						log('CSVideoSelector - newInstance - selectVideo - selected IDs: ', selectedPhotoIds);
						if (typeof instanceSettings.callbackVideoSelected === "function") { instanceSettings.callbackVideoSelected(videoId); }
					} else {
						log('CSVideoSelector - newInstance - selectVideo - ID already stored');
					}
				}

			} else {
				// Remove Video from selectedPhotoIds
				for (i = 0, len = selectedPhotoIds.length; i < len; i += 1) {
					if (selectedPhotoIds[i] === videoId) {
						selectedPhotoIds.splice(i, 1);
						$video.removeClass(settings.albumSelectedClass);
						$selectedCount.html(selectedPhotoIds.length);
						if (typeof instanceSettings.callbackVideoUnselected === "function") { instanceSettings.callbackVideoUnselected(videoId); }
						return false;
					}
				}
			}
	...
	buildAlbumSelector = function(id, callback) {
		...
		FB.getLoginStatus(function(response) {
			if (response.status === 'connected') {
				...
				//videos = [];
				FB.api('/'+ id +'/videos/uploaded', function(response) {
					log('/'+ id +'/videos/uploaded response: ', response);
					if (response.data.length) {
						//addVideoAlbum();
	...
	buildPhotoSelector = function(callback, albumId) {
		...
		log("buildPhotoSelector", albumId);
	...
	/**
	 * Load the Facebook videos and build the markup
	 */
	buildVideoSelector = function(callback, albumId) {
		var buildSecondVideoMarkup, buildVideoMarkup;
		log("buildVideoSelector", albumId);
		...
		FB.api('/me/videos/uploaded', function(response) {
			if (response.data) {
				...
				log('CSPhotoSelector - showPhotoSelector - No videos returned');
				return false;
			}
		});
				
		// Build the markup of the video selector
		buildSecondVideoMarkup = function() {
			//loop through videos
			var i, len, html = '';
			// if video is empty, we need to try again
			log('buildSecondVideoMarkup', videos);
			if (!videos) {
				log('CSPhotoSelector - showVideoSelector - No videos returned');
				return false;
			}
			for (i = 0, len = videos.length; i < len; i += 1) {
				html += buildVideoMarkup(videos[i]);
			}
			
			$videos = $(html);
		};
		
		buildVideoMarkup = function(video) {
			return '<a href="#" class="CSPhotoSelector_photo CSPhotoSelector_clearfix" data-id="' + video.id + '">' +
					'<span><img src="' + video.picture + '" alt="" class="CSPhotoSelector_photoAvatar" /></span>' +
					'</a>';
		};
	};
